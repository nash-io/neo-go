...
func TestInitSmartContract(t *testing.T) {
	contractPath := "testContract"
	defer func() {
		err := os.RemoveAll(contractPath)
		require.NoError(t, err)
	}()

	set := flag.NewFlagSet("flagSet", flag.ExitOnError)
	set.String("name", contractPath, "")
	ctx := cli.NewContext(cli.NewApp(), set, nil)
	initSmartContract(ctx)
	dirInfo, err := os.Stat(contractPath)
	require.NoError(t, err)
	...
	main, err := ioutil.ReadFile(contractPath + "/" + files[0].Name())
	require.NoError(t, err)
	require.Equal(t,
...
func RuntimeNotify(args []interface{}) {
	...
	manifest, err := ioutil.ReadFile(contractPath + "/" + files[1].Name())
	require.NoError(t, err)
	require.Equal(t,
		`hasstorage: false
ispayable: false
